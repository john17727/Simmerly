package dev.juanrincon.simmerly.core.data.remote.utils.dynamic_base_url_ktor_plugin
...
class SupplierBaseUrlConfig {
     ...
     */
    fun loadBaseUrl(block: suspend () -> String?) {
        loadBaseUrl = block
    }
     ...
     */
    fun buildUrl(block: ((URLBuilder, Url) -> URLBuilder)?) {
        buildUrl = block
    }
     ...
     * @param block The lambda to build the final URL.
     */
    fun buildUrlString(block: ((URLBuilder, String) -> URLBuilder)?) {
        buildUrlString = block
    }
}
